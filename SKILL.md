...
- **设计系统从第一天就位**（防日后花一天做令牌收敛手术）：app.wxss `page{}` 令牌层、反馈组件层（app-toast/app-dialog/app-sheet + feedback.js）、弹层编辑器单轨、`docs/design-system.md` 成文——细则见配套 skill `miniprogram-hardening` §7。
...
- **发布加固细则走配套 skill `miniprogram-hardening`**：真机远程调试三件套（构建号上屏/页面级超时兜底/关键反馈页内组件渲染）、发布工程四件套（云函数版本握手/部署诊断全覆盖/环境凭证进 CI/体验版发布 SOP）、小程序特有坑清单与真机验收清单。
